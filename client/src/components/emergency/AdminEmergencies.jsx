...
export default function AdminEmergencies() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        let result = {};
        if ('AdminEmergencies' === 'AdminDashboard' || 'AdminEmergencies' === 'ModeratorDashboard') {
        ...
        } else if ('AdminEmergencies' === 'AdminUsers' || 'AdminEmergencies' === 'ModeratorUsers') {
          const r = await usersAPI.getAll({});
          result = { users: r.data.users||[] };
        } else if ('AdminEmergencies' === 'AdminJobs') {
          const r = await jobsAPI.getAll();
          result = { applications: r.data.applications||[] };
        } else if ('AdminEmergencies' === 'AdminSupport') {
          const r = await supportAPI.getAll();
          result = { tickets: r.data.tickets||[] };
        } else if ('AdminEmergencies' === 'AdminEmergencies') {
          const r = await emergencyAPI.getAll();
          result = { emergencies: r.data.emergencies||[] };
        } else if ('AdminEmergencies' === 'AdminAnalytics') {
          const r = await analyticsAPI.get();
          result = r.data;
        } else if ('AdminEmergencies' === 'TechnicianDashboard' || 'AdminEmergencies' === 'TechnicianTasks') {
          const r = await complaintsAPI.getAll({});
          result = { complaints: r.data.complaints||[] };
        } else if ('AdminEmergencies' === 'EmergencyDashboard' || 'AdminEmergencies' === 'EmergencyAlerts') {
          const r = await emergencyAPI.getAll();
          result = { emergencies: r.data.emergencies||[] };
        } else if ('AdminEmergencies' === 'JobApplication') {
          const r = await jobsAPI.getAll();
          result = { applications: r.data.applications||[] };
        } else if ('AdminEmergencies' === 'SupportPage') {
          const r = await supportAPI.getAll();
          result = { tickets: r.data.tickets||[] };
        }
        setData(result);
    ...
    <>
      ...
      <div className="page-body animate-fadeIn">
        <div className="card card-gold">
          <h2 style={{ fontFamily:'var(--font-display)', fontWeight:700, fontSize:'1.2rem', marginBottom:10 }}>AdminEmergencies</h2>
