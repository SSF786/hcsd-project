...
export default function TechnicianTasks() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        let result = {};
        if ('TechnicianTasks' === 'AdminDashboard' || 'TechnicianTasks' === 'ModeratorDashboard') {
        ...
        } else if ('TechnicianTasks' === 'AdminUsers' || 'TechnicianTasks' === 'ModeratorUsers') {
          const r = await usersAPI.getAll({});
          result = { users: r.data.users||[] };
        } else if ('TechnicianTasks' === 'AdminJobs') {
          const r = await jobsAPI.getAll();
          result = { applications: r.data.applications||[] };
        } else if ('TechnicianTasks' === 'AdminSupport') {
          const r = await supportAPI.getAll();
          result = { tickets: r.data.tickets||[] };
        } else if ('TechnicianTasks' === 'AdminEmergencies') {
          const r = await emergencyAPI.getAll();
          result = { emergencies: r.data.emergencies||[] };
        } else if ('TechnicianTasks' === 'AdminAnalytics') {
          const r = await analyticsAPI.get();
          result = r.data;
        } else if ('TechnicianTasks' === 'TechnicianDashboard' || 'TechnicianTasks' === 'TechnicianTasks') {
          const r = await complaintsAPI.getAll({});
          result = { complaints: r.data.complaints||[] };
        } else if ('TechnicianTasks' === 'EmergencyDashboard' || 'TechnicianTasks' === 'EmergencyAlerts') {
          const r = await emergencyAPI.getAll();
          result = { emergencies: r.data.emergencies||[] };
        } else if ('TechnicianTasks' === 'JobApplication') {
          const r = await jobsAPI.getAll();
          result = { applications: r.data.applications||[] };
        } else if ('TechnicianTasks' === 'SupportPage') {
          const r = await supportAPI.getAll();
          result = { tickets: r.data.tickets||[] };
        }
        setData(result);
    ...
    <>
      ...
      <div className="page-body animate-fadeIn">
        <div className="card card-gold">
          <h2 style={{ fontFamily:'var(--font-display)', fontWeight:700, fontSize:'1.2rem', marginBottom:10 }}>TechnicianTasks</h2>
