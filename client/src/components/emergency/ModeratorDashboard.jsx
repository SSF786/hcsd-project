...
export default function ModeratorDashboard() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        let result = {};
        if ('ModeratorDashboard' === 'AdminDashboard' || 'ModeratorDashboard' === 'ModeratorDashboard') {
        ...
        } else if ('ModeratorDashboard' === 'AdminUsers' || 'ModeratorDashboard' === 'ModeratorUsers') {
          const r = await usersAPI.getAll({});
          result = { users: r.data.users||[] };
        } else if ('ModeratorDashboard' === 'AdminJobs') {
          const r = await jobsAPI.getAll();
          result = { applications: r.data.applications||[] };
        } else if ('ModeratorDashboard' === 'AdminSupport') {
          const r = await supportAPI.getAll();
          result = { tickets: r.data.tickets||[] };
        } else if ('ModeratorDashboard' === 'AdminEmergencies') {
          const r = await emergencyAPI.getAll();
          result = { emergencies: r.data.emergencies||[] };
        } else if ('ModeratorDashboard' === 'AdminAnalytics') {
          const r = await analyticsAPI.get();
          result = r.data;
        } else if ('ModeratorDashboard' === 'TechnicianDashboard' || 'ModeratorDashboard' === 'TechnicianTasks') {
          const r = await complaintsAPI.getAll({});
          result = { complaints: r.data.complaints||[] };
        } else if ('ModeratorDashboard' === 'EmergencyDashboard' || 'ModeratorDashboard' === 'EmergencyAlerts') {
          const r = await emergencyAPI.getAll();
          result = { emergencies: r.data.emergencies||[] };
        } else if ('ModeratorDashboard' === 'JobApplication') {
          const r = await jobsAPI.getAll();
          result = { applications: r.data.applications||[] };
        } else if ('ModeratorDashboard' === 'SupportPage') {
          const r = await supportAPI.getAll();
          result = { tickets: r.data.tickets||[] };
        }
        setData(result);
  ...
  if (loading) return (
    <>
      <Header title="ModeratorDashboard" />
    ...
    <>
      ...
      <div className="page-body animate-fadeIn">
        <div className="card card-gold">
          <h2 style={{ fontFamily:'var(--font-display)', fontWeight:700, fontSize:'1.2rem', marginBottom:10 }}>ModeratorDashboard</h2>
